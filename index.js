...
document.addEventListener('DOMContentLoaded',function(event){
    ...
    function typeWriter(text, i, fnCallback) {
      ...
      if (i < (text.length)) {
       ...
       document.querySelector(".transform-text").innerHTML = text.substring(0, i+1) +'<span class="tranform-text" aria-hidden="true"></span>';
...
text.innerHTML=text.innerText.split('').map((char,i)=>`<span style="transform:rotate(${i*12}deg)">${char}</span>`).join('');
